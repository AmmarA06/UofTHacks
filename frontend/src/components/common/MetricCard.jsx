...
export function MetricCard({ title, value, subtext, trend, className }) {
    return (
        <div className={clsx(
            "bg-[#f3f3f3] rounded-2xl p-6 relative overflow-hidden",
            "hover:bg-[#eeeeee] transition-all duration-200",
            className
        )}>
            <div className="flex flex-col h-full justify-between relative z-10">
                <div>
                    <h3 className="text-[12px] font-medium uppercase tracking-wider text-gray-400 mb-1">{title}</h3>
                    <div className="text-[32px] font-medium text-[#1a1a1a] tracking-[-0.02em] mt-2">{value}</div>
                </div>

                {subtext && (
                    <div className="mt-4 flex items-center text-[13px] text-gray-500">
